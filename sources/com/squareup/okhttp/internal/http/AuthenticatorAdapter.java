...
public final class AuthenticatorAdapter implements Authenticator {
    ...
    private InetAddress getConnectToInetAddress(Proxy proxy, HttpUrl httpUrl) throws IOException {
        return (proxy == null || proxy.type() == Proxy.Type.DIRECT) ? InetAddress.getByName(httpUrl.host()) : proxy.address().getAddress();
    }
    ...
    public Request authenticateProxy(Proxy proxy, Response response) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ("Basic".equalsIgnoreCase(challenge.getScheme())) {
                ...
                PasswordAuthentication requestPasswordAuthentication = java.net.Authenticator.requestPasswordAuthentication(address.getHostName(), getConnectToInetAddress(proxy, httpUrl), address.getPort(), httpUrl.scheme(), challenge.getRealm(), challenge.getScheme(), httpUrl.url(), Authenticator.RequestorType.PROXY);
