...
public final class RealConnection implements Connection {
    ...
    private void connectTls(int i, int i2, ConnectionSpecSelector connectionSpecSelector) throws IOException {
        ...
        try {
            try {
                createSocket = address.getSslSocketFactory().createSocket(this.rawSocket, address.getUriHost(), address.getUriPort(), true);
        ...
        try {
            ConnectionSpec configureSecureSocket = connectionSpecSelector.configureSecureSocket(createSocket);
            if (configureSecureSocket.supportsTlsExtensions()) {
                Platform.get().configureTlsExtensions(createSocket, address.getUriHost(), address.getProtocols());
            }
            createSocket.startHandshake();
            Handshake handshake = Handshake.get(createSocket.getSession());
            if (address.getHostnameVerifier().verify(address.getUriHost(), createSocket.getSession())) {
                ...
                r0 = configureSecureSocket.supportsTlsExtensions() ? Platform.get().getSelectedProtocol(createSocket) : null;
                this.socket = createSocket;
                this.source = Okio.buffer(Okio.source((Socket) createSocket));
                this.sink = Okio.buffer(Okio.sink(this.socket));
                this.handshake = handshake;
                this.protocol = r0 != null ? Protocol.get(r0) : Protocol.HTTP_1_1;
                Platform.get().afterHandshake(createSocket);
                return;
            }
    ...
    public void connect(int i, int i2, int i3, List list, boolean z) throws RouteException {
