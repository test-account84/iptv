        ...
        synchronized (Mimetypes.class) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    HashMap hashMap = mimetypes.extensionToMimetypeMap;
                    for (String str : hashMap.keySet()) {
                        log.debug("Setting mime type for extension '" + str + "' to '" + ((String) hashMap.get(str)) + "'");
                    }
                }
                return mimetypes;
            } catch (Throwable th) {
    ...
    public String getMimetype(String str) {
        ...
        if (lastIndexOf <= 0 || (i = lastIndexOf + 1) >= str.length()) {
            ...
            if (this.extensionToMimetypeMap.containsKey(lowerCase)) {
                String str3 = (String) this.extensionToMimetypeMap.get(lowerCase);
                Log log3 = log;
                if (log3.isDebugEnabled()) {
                    log3.debug("Recognised extension '" + lowerCase + "', mimetype is: '" + str3 + "'");
                }
                return str3;
            }
            log2 = log;
            if (log2.isDebugEnabled()) {
                str2 = "Extension '" + lowerCase + "' is unrecognized in mime type listing, using default mime type: 'application/octet-stream'";
                log2.debug(str2);
            }
        }
