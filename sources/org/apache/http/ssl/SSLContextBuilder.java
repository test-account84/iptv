...
public class SSLContextBuilder {
    ...
    public static class KeyManagerDelegate extends X509ExtendedKeyManager {
        ...
        public Map getClientAliasMap(String[] strArr, Principal[] principalArr) {
        ...
        public Map getServerAliasMap(String str, Principal[] principalArr) {
            HashMap hashMap = new HashMap();
    ...
    public void initSSLContext(SSLContext sSLContext, Collection collection, Collection collection2, SecureRandom secureRandom) throws KeyManagementException {
    ...
    public SSLContextBuilder loadKeyMaterial(KeyStore keyStore, char[] cArr, PrivateKeyStrategy privateKeyStrategy) throws NoSuchAlgorithmException, KeyStoreException, UnrecoverableKeyException {
        ...
        X509ExtendedKeyManager[] keyManagers = keyManagerFactory.getKeyManagers();
        if (keyManagers != null) {
            if (privateKeyStrategy != null) {
                for (int i = 0; i < keyManagers.length; i++) {
                    X509ExtendedKeyManager x509ExtendedKeyManager = keyManagers[i];
                    if (x509ExtendedKeyManager instanceof X509ExtendedKeyManager) {
                        keyManagers[i] = new KeyManagerDelegate(x509ExtendedKeyManager, privateKeyStrategy);
                    }
                }
            }
            for (X509ExtendedKeyManager x509ExtendedKeyManager2 : keyManagers) {
                this.keyManagers.add(x509ExtendedKeyManager2);
            }
        }
        return this;
    }
